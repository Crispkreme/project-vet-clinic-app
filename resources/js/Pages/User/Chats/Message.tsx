...
import { OwnerMessageProps, User } from "@/Interfaces";

const Message: React.FC<OwnerMessageProps> = ({ user, doctors = [], selectedDoctor, flash, sentMessages }) => {

    const owner = usePage<PageProps<{ auth: { user: User } }>>().props.auth.user;
    const doctorProfilePicture = user.profile;

    const { data, setData, post, reset } = useForm<{ message: string }>({
        message: '',
    });
                            ...
                            {sentMessages.map((sentMessage) => (
                                sentMessage.sender !== owner.name ? (
                                    <DoctorReply 
                                        ...
                                        doctors={doctors}
                                        picture={selectedDoctor.profile}
                                    />
                                ) : (
                                    <OwnerReply 
                                        key={sentMessage.id} 
